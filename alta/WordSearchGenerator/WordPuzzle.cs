...
namespace WordSearchGenerator
{
    class WordPuzzle
    {
        ...
        {
            //totalListChars = calculateTotalListChars(ListaCuvinte);
                    ...
                    case orientation.HorFor:
                        {
                            ...
                            while (!okay)
                            {
                                Litera[,] testGrid = CopyArray(grid.puzzle);
                                okay = true;
                                ...
                                for (int i = 0; i < wordChars.Length; i++)
                                {
                                    ...
                                    {
                                        testGrid[startingRow, currentCol] = new Litera(wordChars[i], true);
                                        currentCol++;
                                    }
                                }
                                if (okay)
                    ...
                    case orientation.VertFor:
                        {
                            ...
                            while (!okay)
                            {
                                ...
                                for (int i = 0; i < wordChars.Length; i++)
                                {
                                    ...
                                    {
                                        testGrid[currentRow, startingCol] = new Litera(wordChars[i], true);
                                        currentRow++;
                                    }
                                }
                                if (okay)
                    ...
                    case orientation.SlashFor: //Diagonal word populated up and to the right from the start location
                        {
                            ...
                            while (!okay)
                            {
                                ...
                                for (int i = 0; i < wordChars.Length; i++)
                                {
                                    ...
                                    {
                                        testGrid[currentRow, currentCol] = new Litera(wordChars[i], true);
                                        currentRow--;
                                        currentCol++;
                                    }
                                }
        ...
        private Litera[,] CopyArray(Litera[,] sourceArray)
        {
            int height = sourceArray.GetLength(0);
            int width = sourceArray.GetLength(1);
            Litera[,] copyOfArray = new Litera[height, width];
